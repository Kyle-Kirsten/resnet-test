...
if args.eval:
    ...
    pr.disable()
    # 将结果保存到字符串
    pr.dump_stats('resnet_numpy_profile_results.txt')
    result = io.StringIO()
    ...
    os.system('gprof2dot -f pstats resnet_numpy_profile_results.txt | dot -Tpng -o resnet_numpy_profile_graph.png')
else:
    f=open(f'saves/resnet_numpy_{args.num_epochs}.pkl','rb')
    net=pickle.load(f)
    f.close()
...
# 保存图形
plt.savefig('resnet_numpy_training_metrics.png', dpi=240, bbox_inches='tight')
