...
class fc_softmax:
    ...
    def forward(self, in_tensor):
        ...
        self.out_tensor = np.exp(self.out_tensor) / np.sum(np.exp(self.out_tensor), axis=1, keepdims=True)
    ...
    def backward(self, out_diff_tensor, lr):
        ...
        s_dot_grad = np.sum(self.out_tensor * out_diff_tensor, axis=1, keepdims=True)
        nonlinear_diff = self.out_tensor * (out_diff_tensor - s_dot_grad)
...
class resnet9_numpy:
    ...
    def __init__(self, num_classes=10):
        self.layer0 = [
            ...
            bn_layer(128),
            relu(),
            max_pooling(2,2,2,same=True)
        ]
        self.layer1 = [ResBlock(128, 128, stride=1, shortcut=None)]
        self.layer2 = [
    ...
    def backward(self, out_diff_tensor, lr):
        ...
        self.max_pooling.backward(x, lr)
        x = self.max_pooling.in_diff_tensor
        ...
        for l in range(1, len(self.layer0)+1):
            self.layer0[-l].backward(x, lr)
            x = self.layer0[-l].in_diff_tensor
